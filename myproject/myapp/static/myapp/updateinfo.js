document.addEventListener('DOMContentLoaded', (event)=>{
    ...
    const validate_latest_username= () => {
        ...
        fetch(`http://172.17.0.1:80/validate-latest-username/?username=${encodeURIComponent(usernameElement.value)}`).then(response=> response.json())
        .then(data => {
    ...
    const validate_latest_email = () => {
        ...
        fetch(`http://172.17.0.1:8000/validate-latest-email/?email=${encodeURIComponent(emailElement.value)}`).then(response=> response.json())
        .then(data => {
